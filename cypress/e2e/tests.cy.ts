describe('Тесты конструктора', ()=>{
    ...
    it('По клику на ингриденте открывается окно c компонентом', () => {
        ...
        cy.get('[data-cy=modal]').contains('Биокотлета из марсианской Магнолии тест');
    });
...
describe('Тесты профиля', () => {
    ...
    const checkFormInOriginalState = () => {
...
describe('Заказ', () => {
    beforeEach(() => {
        ...
        cy.intercept("POST", "/api/orders", (req) => {
            ...
            } else if (req.body.ingredients.length === 3 &&
